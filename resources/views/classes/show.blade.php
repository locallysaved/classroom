<x-app-layout>
    ...
    <div style="max-width:1100px;margin:0 auto;">
        ...
        <div style="display:grid;grid-template-columns:1fr 290px;align-items:start;gap:24px;">
            ...
            <div style="display:flex;flex-direction:column;gap:20px;">
                ...
                @if($class->description)
                    <div class="card card-body"
                        ...
                        @if(auth()->user()->isAdmin() || auth()->user()->id === $class->user_id)
                            <form action="{{ route('classes.destroy', $class) }}" method="POST"
                                  onsubmit="return confirm('Delete this class? This will remove all tasks and files permanently.');"
                                  style="flex-shrink:0;">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                        style="background:rgba(0,0,0,.08);color:var(--text);border:none;
                                               border-radius:8px;padding:7px 14px;font-size:13px;font-weight:600;
                                               font-family:inherit;cursor:pointer;transition:background .15s;"
                                        onmouseenter="this.style.background='#fee2e2';this.style.color='#dc2626'"
                                        onmouseleave="this.style.background='rgba(0,0,0,.08)';this.style.color='var(--text)'">
                                    Delete Class
                                </button>
                            </form>
                        @endif
                                        ...
                                        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;">
                                            <div style="flex:1;">
                                                ...
                                                <p style="font-size:13px;color:var(--muted);line-height:1.5;
                                                          padding-left:16px;margin:0;overflow:hidden;
                                            ...
                                            <div style="display:flex;flex-direction:column;align-items:flex-end;gap:5px;flex-shrink:0;">
                                                ...
                                                @if($task->due_date)
                                                    <span style="font-size:12px;white-space:nowrap;padding:3px 10px;
            ...
            {{-- Right column — join code sidebar --}}
                ...
                <div style="position:sticky;top:24px;display:flex;flex-direction:column;gap:16px;">
                    <div class="card card-body">
                        ...
                        <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;">
                            <span style="font-family:monospace;font-size:22px;font-weight:800;
                                         letter-spacing:.15em;color:var(--red);background:var(--bg);
                                         padding:8px 14px;border-radius:8px;border:2px solid var(--red);
                        ...
                        <p id="copied-msg" style="display:none;font-size:12px;color:#059669;margin-bottom:12px;">
                            Copied to clipboard!
                        </p>
                        ...
                        <div style="background:#fff;border:1px solid var(--border);border-radius:10px;
                            ...
                            <img src="https://api.qrserver.com/v1/create-qr-code/?size=180x180&data={{ urlencode(url('/join/' . $class->join_code)) }}"
                                 alt="QR Code"
                                 style="width:100%;max-width:180px;height:auto;border-radius:6px;display:block;">
                        </div>
